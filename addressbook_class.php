<?php
	...
	class addressbook_service{
		...
		protected function escapeString($t,$is_sqlite=true){
			if($is_sqlite) return $this->db->escapeString($t);
			return $this->db->real_escape_string($t);
		}

		public function __construct($dbname='addressbook.db'){
			try{
		...
		protected function createTablesIfNotExists(){
			$this->db->exec('CREATE TABLE IF NOT EXISTS domains (host varchar(255), description TEXT, b64 TEXT);');
			$this->db->exec('CREATE TABLE IF NOT EXISTS tmp_files (host varchar(255), namefile varchar(255));');
		...
		public function deleteDomain($domain){
			$domain = $this->escapeString($domain);
			$this->db->exec("DELETE FROM domains where host='$domain';");
		}
		...
		protected function genTmpFileForDomain($host){
			...
			$this->db->exec("INSERT INTO tmp_files(host,namefile) VALUES('$host','$tmpString');");
			return $tmpString;
		}
		...
		public function deleteTmpFilesForDomains(){
			$this->db->exec('DELETE FROM tmp_files;');
		}
		protected function deleteTmpFilesForDomain($host){
			$host = $this->escapeString($host);
			$this->db->exec("DELETE FROM tmp_files where host='$host';");
		}	
		public function regSubDomain($domain,$b64,$description,$proxy='tcp://127.0.0.1:4444'){
			...
			if( $r ){ // file exists already
				...
				$file=$this->getFileThoughProxy("http://$host/$namefile",$proxy);
				if($file === false){
